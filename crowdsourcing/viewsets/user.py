...
class UserViewSet(mixins.RetrieveModelMixin, mixins.CreateModelMixin, mixins.UpdateModelMixin,
    ...
    def available_workers(self, request, *args, **kwargs):
        ...
        #return Response({"count": workers[0]['worker__count'] if len(workers) else 1}) ### Bir - Change to send hard coded 10 active users 
        return Response({"count": 10 if 10 else 1})
...
class UserProfileViewSet(mixins.RetrieveModelMixin,
    ...
    def stripe(self, request, *args, **kwargs):    #saves credit card info to stripe, validates and approves as worker / requester
    ...
    def financial_data(self, request):   #another stripe related function - keeps track of payment related - calculate the amount for the user
        profile = request.user.profile
        ...
        response_data.update({"account_balance": 500})
        ...
        response_data.update({"default_card": "123123123123123123"})
        response_data.update({"default_bank": "HANA BANK"})
